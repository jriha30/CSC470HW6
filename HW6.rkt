...
(define no-parser
  (lambda (no-code)
    (cond
      ((eq? no-code 'True) 'True)
      ((eq? no-code 'False) 'False)
      ((number? no-code) (list 'num-lit-exp no-code))
      ((symbol? no-code) (list 'var-exp no-code))
      ((eq? (car no-code) 'if)
       (list 'if-exp (no-parser (caadr no-code)) (list 'true-exp (no-parser (caddr no-code))) (list 'false-exp (no-parser (cadddr no-code)))))
...
(define sample-no-code '(call (function (x y) (do-mathy-stuff + x y)) (if (True) (do-mathy-stuff + age b) (do-mathy-stuff - 7 b)) (do-mathy-stuff + 7 c)))
